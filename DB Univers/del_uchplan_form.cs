...
{
    public partial class del_uchplan_form : Form
    {
        string connect, query;
        DataSet ds;
        DataTable dt;
        SqlDataAdapter adapterSql;
        public del_uchplan_form(Form1 main_form)
        {
            InitializeComponent();
            ...
            else
            {
                ...
                while (i < dt.Rows.Count)
                {
                    ...
                    if (comboBox1.Text == st)
                    {
                        ...
                        query = "DELETE FROM [Учебный план] WHERE [ID УП]='" + comboBox1.Text + "';";
                        adapterSql = new SqlDataAdapter(query, connect);
                        adapterSql.Fill(ds);
                        dt.Dispose();
                        ds.Dispose();
        ...
        private void del_uchplan_form_Load(object sender, EventArgs e)
        {
            dt = new DataTable();
            ds = new DataSet();
            query = "SELECT [ID УП] From [Учебный план]";

...
namespace DB_Univers
{
    ...
    public partial class add_grp_form : Form
    {
        string connect, query;
        DataSet ds;
        DataTable dt;
        SqlDataAdapter adapterSql;
        public add_grp_form(Form1 main_form)
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    st = Convert.ToString(ds.Tables[0].Rows[i].ItemArray[0]);
                    if (textBox1.Text == st)
                    {
                ...
                query = "insert into Группа([№ группы],[Название института]) values('" + textBox1.Text + "','"+comboBox1.Text+"');";
                adapterSql = new SqlDataAdapter(query, connect);
                adapterSql.Fill(ds);
                dt.Dispose();
                ds.Dispose();
        ...
        private void add_grp_form_Load(object sender, EventArgs e)
        {
            dt = new DataTable();
            ds = new DataSet();

...
{
    public partial class add_auditor_form : Form
    {
        string connect, query;
        DataSet ds;
        DataTable dt;
        SqlDataAdapter adapterSql;
        public add_auditor_form(Form1 main_form)
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                while (i < dt.Rows.Count)
                {
                    ...
                    if (textBox1.Text == st)
                    {
                        MessageBox.Show("Такая аудитория уже внесена");
                        return;
                    }
                    i++;
                }

                query = "insert into Аудитория([№ аудитории]) values('"  + textBox1.Text + "');";
                adapterSql = new SqlDataAdapter(query, connect);
                adapterSql.Fill(ds);
                dt.Dispose();
                ds.Dispose();
        ...
        private void add_auditor_form_Load(object sender, EventArgs e)
        {
            query = "SELECT * from Аудитория";
            adapterSql = new SqlDataAdapter(query, connect);
            ds = new DataSet();
